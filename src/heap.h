...
typedef long heapvalues; /* key values */
...
//void HeapPrint(Heap *h)
{
   ...
   fprintf(stderr, "\nTHE HEAP: size %d of %d", Size, h->N);
   for ( iNode = 0; iNode < Size; iNode++ )
   {
      fprintf(stderr,"\npos %d (%d,%lf): ",iNode,Node[iNode],key[Node[iNode]]);
      for ( iChild = (iNode << LOG_HEAP_DEGREE) + 1;
            iChild <= (iNode+1) << LOG_HEAP_DEGREE  &&  iChild < Size;
            iChild++ )
      {
         fprintf(stderr, "(%d,%lf) ", Node[iChild],key[Node[iChild]]);
...
{	
	...
	for ( iCurrent = Pos[v];
		/* break when value <= min */; iCurrent = iGoodChild )
	{
		iFirstChild = (iCurrent << LOG_HEAP_DEGREE) + 1;
		iLastChild = MIN(iFirstChild + (1<<LOG_HEAP_DEGREE) - 1, h->Size-1);
		if ( iFirstChild >= h->Size )                 /* at the end of the tree*/
	...
	PLACE_NODE(v, iCurrent);                /* Node goes in hole*/
...
/* TODO: function added by Daniel Kuck - 06/07/2009 */
/*inline static void adjustHeap(Heap *h, heapindices v)
{
